...
Easystroke has been in abandoned state. There appears to be a numer of forks circulating on google with patches provided by humble easystroke users. Of the repositories I found, this one had the most complete information and compiled fine on my system.

[https://github.com/thjaeger/easystroke](https://github.com/thjaeger/easystroke)

`thejaeger` repository had some files but then it would ask you to download files from another server to compile, which are not on github. This repository is an effort to combine all the information there and the external files from `openartisthq.org`.

I have only tested `patched-easystroke-master.tar.bz2`.  Everything compiled fine on my arch distro. I kept the other files for consistency.
